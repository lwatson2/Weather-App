 ...
 */
  ...
  FetchCurrentWeather(props) {
    fetch(
      `${API_URL}weather?q=${this.state.searchQuery}&APPID=${
  ...
  FetchFiveWeather(props) {
    fetch(
      `${API_URL}forecast?q=${this.state.searchQuery}&APPID=${
        process.env.REACT_APP_WEATHER_API_KEY
      }&units=imperial`
    )
      .then(response => response.json())
  ...
  FetchUserFiveDay(props) {
    fetch(
      `${API_URL}forecast?lat=${this.state.lat}&lon=${this.state.lon}&APPID=${
        process.env.REACT_APP_WEATHER_API_KEY
      }&units=imperial`
    )
      .then(response => response.json())
  ...
  FetchUserWeather(props) {
    fetch(
      `${API_URL}/weather?lat=${this.state.lat}&lon=${this.state.lon}&APPID=${
        process.env.REACT_APP_WEATHER_API_KEY
      }&units=imperial`
    )
      .then(response => response.json())
      ...
      return (
        <div className="App">
          ...
          <Current
            ...
            name={name}
          />
          <ListHandler fivedayforecast={fivedayforecast} />
        </div>
      );
    }
  }
